...
const mockDatabase = {
    ...
    workers: [
        { id: 101, name: 'Ramesh Kumar', role: 'Field Supervisor', perDaySalary: 800, daysPresentThisWeek: 5, loanBalance: 2000 },
        { id: 102, name: 'Sunita Devi', role: 'Harvester', perDaySalary: 600, daysPresentThisWeek: 6, loanBalance: 0 },
        { id: 103, name: 'Amit Singh', role: 'Irrigation Specialist', perDaySalary: 750, daysPresentThisWeek: 4, loanBalance: 500 },
        { id: 104, name: 'Priya Sharma', role: 'Harvester', perDaySalary: 600, daysPresentThisWeek: 5, loanBalance: 3500 },
        { id: 105, name: 'Vikram Choudhary', role: 'Tractor Operator', perDaySalary: 900, daysPresentThisWeek: 6, loanBalance: 0 },
    ],
    financials: {
    ...
    financialChartData: [ { name: 'May', revenue: 50000, expenses: 30000 }, { name: 'Jun', revenue: 65000, expenses: 40000 }, { name: 'Jul', revenue: 90000, expenses: 55000 }, { name: 'Aug', revenue: 75000, expenses: 50000 }, { name: 'Sep', revenue: 89800, expenses: 45000 }, ]
};
...
const yieldService = { getYields: () => api.get(API_ENDPOINTS.YIELDS), getYieldChartData: () => Promise.resolve(mockDatabase.yieldChartData), };
const workerService = { getWorkers: () => api.get(API_ENDPOINTS.WORKERS) };
const financialService = { getFinancials: () => api.get(API_ENDPOINTS.FINANCIALS), getFinancialChartData: () => Promise.resolve(mockDatabase.financialChartData), };
...
const useData = (serviceFn) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setIsLoading(true);
            try { const result = await serviceFn(); if(isMounted) setData(result); } catch (err) { if(isMounted) setError(err); } finally { if(isMounted) setIsLoading(false); }
        };
        fetchData();
...
const YieldModal = ({ isOpen, onClose, onSave, yieldToEdit, cropOptions }) => {
    ...
    useEffect(() => {
        if (yieldToEdit) { setFormData(yieldToEdit); } else { setFormData({ date: new Date().toISOString().split('T')[0], crop: cropOptions[0] || '', quantity: '', unit: 'kg', grade: 'A', revenue: '' }); }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center">
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit}><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"><div><label className="block text-sm font-medium text-gray-700">Date</label><input type="date" name="date" value={formData.date || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" required /></div><div><label className="block text-sm font-medium text-gray-700">Crop</label><select name="crop" value={formData.crop || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{cropOptions.map(crop => <option key={crop} value={crop}>{crop}</option>)}</select></div><div><label className="block text-sm font-medium text-gray-700">Quantity</label><input type="number" name="quantity" value={formData.quantity || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" required /></div><div><label className="block text-sm font-medium text-gray-700">Unit</label><input type="text" name="unit" value={formData.unit || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" required /></div><div><label className="block text-sm font-medium text-gray-700">Grade</label><select name="grade" value={formData.grade || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"><option value="A">A</option><option value="B">B</option><option value="C">C</option></select></div><div><label className="block text-sm font-medium text-gray-700">Revenue (₹)</label><input type="number" name="revenue" value={formData.revenue || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" required /></div></div><div className="flex justify-end space-x-4 mt-8"><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button type="submit" className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Save</button></div></form>
            </div>
        </div>
    );
};
...
const Dashboard = () => {
    ...
    return (
        <div className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><StatCard title="Total Revenue" value={`₹${financials.summary.revenue.toLocaleString('en-IN')}`} icon={<RevenueIcon />} /><StatCard title="Total Expenses" value={`₹${financials.summary.expenses.toLocaleString('en-IN')}`} icon={<ExpenseIcon />} /><StatCard title="Net Profit" value={`₹${financials.summary.profit.toLocaleString('en-IN')}`} icon={<ProfitIcon />} /></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card title="Yield Performance (Last 5 Months)"><ResponsiveContainer width="100%" height={300}><BarChart data={yieldChartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Legend /><Bar dataKey="Tomatoes" fill="#8884d8" /><Bar dataKey="Potatoes" fill="#82ca9d" /></BarChart></ResponsiveContainer></Card><Card title="Financial Overview (Last 5 Months)"><ResponsiveContainer width="100%" height={300}><LineChart data={financialChartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Legend /><Line type="monotone" dataKey="revenue" stroke="#8884d8" activeDot={{ r: 8 }} /><Line type="monotone" dataKey="expenses" stroke="#82ca9d" /></LineChart></ResponsiveContainer></Card></div></div>
    );
};
const YieldTracking = () => {
    ...
    useEffect(() => { if (initialData) { setYields(initialData.yields); } }, [initialData]);
    const handleOpenModal = (yieldData = null) => { setYieldToEdit(yieldData); setIsModalOpen(true); };
    const handleCloseModal = () => { setYieldToEdit(null); setIsModalOpen(false); };
    const handleSaveYield = (yieldData) => { if (yieldData.id) { setYields(yields.map(y => y.id === yieldData.id ? yieldData : y)); } else { const newYield = { ...yieldData, id: Date.now() }; setYields([newYield, ...yields]); } handleCloseModal(); };
    const handleDeleteYield = (yieldId) => { setYields(yields.filter(y => y.id !== yieldId)); };
    if (isLoading) return <Spinner />;
    return (<><YieldModal isOpen={isModalOpen} onClose={handleCloseModal} onSave={handleSaveYield} yieldToEdit={yieldToEdit} cropOptions={initialData?.cropOptions || []} /><Card title="Yield Records" titleActions={<button onClick={() => handleOpenModal()} className="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700">Add New Yield</button>}><div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-gray-100"><th className="p-3">Date</th><th className="p-3">Crop</th><th className="p-3">Quantity</th><th className="p-3">Grade</th><th className="p-3">Revenue</th><th className="p-3">Actions</th></tr></thead><tbody>{yields.map(y => (<tr key={y.id} className="border-b"><td className="p-3">{y.date}</td><td className="p-3">{y.crop}</td><td className="p-3">{y.quantity} {y.unit}</td><td className="p-3"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${y.grade === 'A' ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800'}`}>{y.grade}</span></td><td className="p-3">₹{y.revenue.toLocaleString('en-IN')}</td><td className="p-3"><div className="flex space-x-2"><button onClick={() => handleOpenModal(y)} className="text-blue-600 hover:text-blue-800"><EditIcon /></button><button onClick={() => handleDeleteYield(y.id)} className="text-red-600 hover:text-red-800"><DeleteIcon /></button></div></td></tr>))}</tbody></table></div></Card></>);
};
const WorkerManagement = () => {
    const { data: initialWorkers, isLoading } = useData(workerService.getWorkers);
    const [workers, setWorkers] = useState([]);
    // State to manage input fields for advance and repayment for each worker
    ...
    // Handle changes in the advance/repayment input fields
    ...
    // Finalize payment, update loan balance, and reset inputs
    const handleConfirmPayment = (workerId) => {
        ...
        // Prevent repaying more than the loan balance + weekly salary
        const maxRepayment = worker.loanBalance;
        if (repayment > maxRepayment) {
            alert(`Repayment cannot exceed the outstanding loan of ₹${maxRepayment.toLocaleString('en-IN')}`);
            return;
        }
        ...
        // Reset the input fields for this worker
        setAdjustments(prev => ({
            ...prev,
            [workerId]: { advance: '', repayment: '' }
        }));
    };
    ...
    return (
         <Card title="Weekly Worker Payments">
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="bg-gray-100">
                            <th className="p-3">Worker</th>
                            <th className="p-3">Outstanding Loan (₹)</th>
                            <th className="p-3">Weekly Base Salary (₹)</th>
                            <th className="p-3">Add Advance / Deduct Loan (₹)</th>
                            <th className="p-3 font-bold">This Week's Payout (₹)</th>
                            <th className="p-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {workers.map(w => {
                            const baseSalary = w.perDaySalary * w.daysPresentThisWeek;
                            ...
                            return (
                                <tr key={w.id} className="border-b">
                                    ...
                                    <td className="p-3">{baseSalary.toLocaleString('en-IN')} <span className="text-xs text-gray-500">({w.daysPresentThisWeek} days)</span></td>
                                    <td className="p-3">
                                        <div className="flex space-x-2">
                                            <input type="number" placeholder="Advance" value={adjustments[w.id]?.advance || ''} onChange={(e) => handleAdjustmentChange(w.id, 'advance', e.target.value)} className="w-24 p-1 border rounded" />
                                            <input type="number" placeholder="Repay" value={adjustments[w.id]?.repayment || ''} onChange={(e) => handleAdjustmentChange(w.id, 'repayment', e.target.value)} className="w-24 p-1 border rounded" />
                                        </div>
                                    </td>
                                    <td className="p-3 font-bold text-lg text-blue-600">{finalPayout.toLocaleString('en-IN')}</td>
                                    <td className="p-3">
                                        <button onClick={() => handleConfirmPayment(w.id)} className="px-3 py-1 text-sm bg-green-600 text-white rounded hover:bg-green-700">
                                            Confirm
                                        </button>
                                    </td>
                                </tr>
