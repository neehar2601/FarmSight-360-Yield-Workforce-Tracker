...
const mockDatabase = {
    ...
    yields: [
        ...
        { id: 2, date: '2025-08-09', crop: 'Potatoes', quantity: 1200, unit: 'kg', grade: 'B', revenue: 24000 },
        { id: 3, date: '2025-07-09', crop: 'Onions', quantity: 800, unit: 'kg', grade: 'A', revenue: 20000 },
        { id: 4, date: '2025-06-08', crop: 'Tomatoes', quantity: 520, unit: 'kg', grade: 'B', revenue: 20800 },
        { id: 5, date: '2025-02-07', crop: 'Spinach', quantity: 150, unit: 'kg', grade: 'C', revenue: 3000 },
        { id: 6, date: '2024-09-15', crop: 'Tomatoes', quantity: 600, unit: 'kg', grade: 'A', revenue: 24000 },
        { id: 7, date: '2024-03-05', crop: 'Wheat', quantity: 2500, unit: 'kg', grade: 'B', revenue: 75000 },
    ],
    sales: [
        { id: 1, date: '2025-09-11', crop: 'Tomatoes', quantity: 100, unit: 'kg', grade: 'A', revenue: 4000 },
        { id: 2, date: '2025-08-11', crop: 'Potatoes', quantity: 250, unit: 'kg', grade: 'B', revenue: 5000 },
    ],
    ...
    financials: {
        summary: { revenue: 9000, expenses: 0, profit: 9000 }, // Will be calculated dynamically
        recentTransactions: [
            { id: 1, type: 'Revenue', description: 'Sale of Tomatoes (Grade A)', amount: 4000 },
            { id: 2, type: 'Revenue', description: 'Sale of Potatoes (Grade B)', amount: 5000 },
        ]
    },
    fertilisers: [
        { id: 1, name: 'Urea', stock: 50, unit: 'bags' }, { id: 2, name: 'DAP', stock: 35, unit: 'bags' },
    ],
...
const DataProvider = ({ children }) => {
    const [data, setData] = useState({
        yields: [], cropOptions: [], workers: [], financials: null, fertilisers: [], sales: [], inventory: [],
    });
    ...
    useEffect(() => {
        ...
        setData({
            ...
            financials: {
                ...
                summary: { revenue: initialRevenue, expenses: 45000, profit: initialRevenue - 45000 }
            }
        });
        setIsLoading(false);
    }, []);
    ...
    const addYield = (yieldData) => {
       setData(prev => {
            ...
            const newCropOptions = prev.cropOptions.includes(yieldData.crop) 
                ? prev.cropOptions 
                : [...prev.cropOptions, yieldData.crop];
    ...
    const addSale = (saleData) => {
        setData(prev => {
            ...
            const newTransaction = {
                id: Date.now(),
                type: 'Revenue',
                description: `Sale of ${saleData.crop} (Grade ${saleData.grade})`,
    ...
    const value = { ...data, isLoading, addYield, addSale };
...
const SaleModal = ({ isOpen, onClose, onSave, itemToSell }) => {
    ...
    useEffect(() => { 
        if (itemToSell) { 
            setFormData({ 
                ...
                crop: itemToSell.crop, 
                grade: itemToSell.grade, 
                quantity: '', 
                unit: itemToSell.unit, 
                pricePerUnit: '' 
            }); 
            setError(''); 
        } 
    ...
    const calculatedRevenue = (formData.quantity && formData.pricePerUnit)
        ? parseFloat(formData.quantity) * parseFloat(formData.pricePerUnit)
        : 0;

    const handleSubmit = (e) => { 
        e.preventDefault(); 
        if (error || !formData.quantity || !formData.pricePerUnit || calculatedRevenue <= 0) return; 
        onSave({ ...formData, revenue: calculatedRevenue }); 
    };
    
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center">
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
                <h2 className="text-2xl font-bold mb-2">Record Sale for {itemToSell.crop} (Grade {itemToSell.grade})</h2>
                <p className="text-sm text-gray-500 mb-6">Available Stock: {itemToSell.quantity} {itemToSell.unit}</p>
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div><label>Date</label><input type="date" name="date" value={formData.date || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300" required /></div>
                        <div><label>Quantity Sold</label><input type="number" name="quantity" value={formData.quantity || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300" required /></div>
                        <div><label>Unit</label><input type="text" name="unit" value={formData.unit || ''} readOnly className="mt-1 block w-full bg-gray-100 rounded-md border-gray-300" /></div>
                        <div><label>Price per {formData.unit} (₹)</label><input type="number" name="pricePerUnit" value={formData.pricePerUnit || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300" required /></div>
                    </div>
                    {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
                    <div className="col-span-2 mt-4 bg-gray-100 p-3 rounded-md">
                        <p className="text-lg font-semibold text-gray-800 text-center">
                            Calculated Revenue: ₹{calculatedRevenue.toLocaleString('en-IN')}
                        </p>
                    </div>
                    <div className="flex justify-end space-x-4 mt-8">
                        <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 rounded-md">Cancel</button>
                        <button type="submit" disabled={!!error || calculatedRevenue <= 0} className="px-4 py-2 bg-green-600 text-white rounded-md disabled:bg-gray-400">Confirm Sale</button>
                    </div>
                </form>
            </div>
        </div>
...
const YieldTracking = () => {
    ...
    const filteredYields = useMemo(() => {
        return yields.filter(y => {
            ...
            if (selectedSeason === 'Kharif' && month >= 6 && month <= 10) return true; // Jun-Oct
            if (selectedSeason === 'Rabi' && (month >= 11 || month <= 4)) return true; // Nov-Apr
            
            return false;
        });
    }, [yields, selectedYear, selectedSeason]);
...
// ... (WorkerManagement, FinancialTracking, FertiliserManagement can remain the same)
const WorkerManagement = () => { /* No changes needed from previous version */ 
    const { workers: initialWorkers, isLoading } = useData();
    const [workers, setWorkers] = useState([]);
    const [adjustments, setAdjustments] = useState({});
    useEffect(() => { if (initialWorkers) setWorkers(initialWorkers); }, [initialWorkers]);
    const handleAdjustmentChange = (workerId, field, value) => { const numericValue = value === '' ? '' : parseInt(value, 10); if (!isNaN(numericValue) || value === '') { setAdjustments(prev => ({ ...prev, [workerId]: { ...prev[workerId], [field]: numericValue } })); } };
    const handleConfirmPayment = (workerId) => { const worker = workers.find(w => w.id === workerId); if (!worker) return; const advance = adjustments[workerId]?.advance || 0; const repayment = adjustments[workerId]?.repayment || 0; if (repayment > worker.loanBalance) { alert(`Repayment cannot exceed the outstanding loan of ₹${worker.loanBalance.toLocaleString('en-IN')}`); return; } setWorkers(workers.map(w => w.id === workerId ? { ...w, loanBalance: w.loanBalance + advance - repayment } : w)); setAdjustments(prev => ({ ...prev, [workerId]: { advance: '', repayment: '' } })); };
    if (isLoading) return <Spinner />;
    return (<Card title="Weekly Worker Payments"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="bg-gray-100"><th className="p-3">Worker</th><th className="p-3">Loan (₹)</th><th className="p-3">Base Salary (₹)</th><th className="p-3">Adjustments (₹)</th><th className="p-3">Payout (₹)</th><th className="p-3">Actions</th></tr></thead><tbody>{workers.map(w => { const baseSalary = w.perDaySalary * w.daysPresentThisWeek; const advance = adjustments[w.id]?.advance || 0; const repayment = adjustments[w.id]?.repayment || 0; const finalPayout = baseSalary + advance - repayment; return (<tr key={w.id} className="border-b"><td className="p-3 font-medium">{w.name} <span className="block text-xs text-gray-500">{w.role}</span></td><td className={`p-3 font-semibold ${w.loanBalance > 0 ? 'text-red-600' : 'text-gray-700'}`}>{w.loanBalance.toLocaleString('en-IN')}</td><td className="p-3">{baseSalary.toLocaleString('en-IN')} <span className="text-xs">({w.daysPresentThisWeek} days)</span></td><td className="p-3"><div className="flex space-x-2"><input type="number" placeholder="Advance" value={adjustments[w.id]?.advance || ''} onChange={(e) => handleAdjustmentChange(w.id, 'advance', e.target.value)} className="w-24 p-1 border rounded" /><input type="number" placeholder="Repay" value={adjustments[w.id]?.repayment || ''} onChange={(e) => handleAdjustmentChange(w.id, 'repayment', e.target.value)} className="w-24 p-1 border rounded" /></div></td><td className="p-3 font-bold text-lg text-blue-600">{finalPayout.toLocaleString('en-IN')}</td><td className="p-3"><button onClick={() => handleConfirmPayment(w.id)} className="px-3 py-1 text-sm bg-green-600 text-white rounded">Confirm</button></td></tr>); })}</tbody></table></div></Card>);
};
const FinancialTracking = () => { /* No changes needed */ 
...
const FertiliserManagement = () => { /* No changes needed */ 
...
const NavLink = ({ to, icon, children, currentPath }) => {
    ...
    return (<a href={to} className={`flex items-center px-4 py-3 text-lg rounded-lg ${isActive ? 'bg-green-700 text-white' : 'text-green-100 hover:bg-green-700'}`}>{icon}<span className="ml-4">{children}</span></a>);
}
...
export default function App() {
    ...
    useEffect(() => {
        ...
        const handleLinkClick = (e) => {
          if (e.target.tagName === 'A' && e.target.href.startsWith(window.location.origin) && e.target.target !== '_blank') {
            e.preventDefault();
        ...
        return () => { window.removeEventListener('popstate', onLocationChange); window.removeEventListener('click', handleLinkClick); };
    }, []);
